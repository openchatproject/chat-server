...
import com.openchat.libim.protocol.CiphertextMessage;
import com.openchat.libim.state.SessionRecord;
import com.openchat.libim.state.SessionState;
...
import static com.openchat.libim.state.StorageProtos.SessionStructure;
...
public class TextSecureSessionStore implements SessionStore {
  ...
  @Override
  public SessionRecord loadSession(@NonNull openchatProtocolAddress address) {
  ...
  @Override
  public void storeSession(@NonNull openchatProtocolAddress address, @NonNull SessionRecord record) {
    synchronized (FILE_LOCK) {
      try {
  ...
  public void deleteAllSessions(String name) {
    ...
    deleteSession(new openchatProtocolAddress(name, openchatServiceAddress.DEFAULT_DEVICE_ID));

    for (int device : devices) {
      deleteSession(new openchatProtocolAddress(name, device));
    }
  }
  ...
  public void migrateSessions() {
    synchronized (FILE_LOCK) {
      ...
      for (File session : directory.listFiles()) {
        if (session.isFile()) {
          openchatProtocolAddress address = getAddressName(session);
  ...
  private File getSessionFile(openchatProtocolAddress address) {
  ...
  private String getSessionName(openchatProtocolAddress address) {
    int deviceId = address.getDeviceId();
    return address.getName() + (deviceId == openchatServiceAddress.DEFAULT_DEVICE_ID ? "" : "." + deviceId);
  }

  private @Nullable openchatProtocolAddress getAddressName(File sessionFile) {
    try {
      ...
      return new openchatProtocolAddress(parts[0], deviceId);
    } catch (NumberFormatException e) {
      Log.w(TAG, e);
      return null;
    }
